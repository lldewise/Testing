...
const CalloutCoverExample: React.FunctionComponent = () => {
  const [isCalloutVisible, { toggle: toggleIsCalloutVisible }] = useBoolean(false);
  const [directionalHint, setDirectionalHint] = React.useState(DirectionalHint.bottomLeftEdge);
  const onDirectionalChanged = (event: React.FormEvent, option: IDropdownOption): void => {
    setDirectionalHint(option.key);
  };

  return (
    <>
    <div className="fluenttitle divpadt10">Callout that covers the target element</div>

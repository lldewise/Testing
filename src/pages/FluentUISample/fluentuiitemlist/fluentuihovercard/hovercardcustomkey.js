...
const onRenderCompactCard = (item: IExampleItem): JSX.Element => {
    return (
        <div className={classNames.compactCard}>
            <a href={`http://wikipedia.org/wiki/${item.location}`}>
                {item.location}
            </a>
        </div>
    );
...
const KeyCellWithHoverCard: React.FunctionComponent<{ item: IExampleItem }> = ({ item }) => {
    ...
    const targetElementRef: React.RefObject<HTMLDivElement> = React.useRef(null);
...
const FluentHoverCardTargetCustom: React.FunctionComponent = () => {
    return (
        <div className="ms-Grid-row" >
            <div className="ms-Grid-col ms-lg12 ">
                <div className="fluenttitle divpadt10">Expanding HoverCard using target, DirectionalHint and custom hotkey</div>
